...
lines.append("- **Source**: where the icon file was originally created (`doxx-www` = copied from web repo, `style (created)` = SVG created in this repo)")
lines.append("")
...
for group_name in sorted(groups.keys()):
    ...
    for rel, full in items:
        ...
        if has_ref:
            ...
            stats_badge = "no"
            no_ref_count += 1
        lines.append(f"| <img src=\"{rel}\" width=\"24\" height=\"24\"> | `{name}` | {et}{('.' + cat) if cat else ''} | {label} | {color_badge} | {stats_badge} | {source} |")
    lines.append("")

lines.append("## Summary")
lines.append("")
lines.append(f"- **{has_ref_count}** icons referenced in `stats_definitions.json` (icon_type: image)")
lines.append(f"- **{no_ref_count}** icons not yet referenced (need API update or are generic/utility icons)")
lines.append("")
