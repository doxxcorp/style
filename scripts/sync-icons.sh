...
for group_name in sorted(groups.keys()):
    ...
    for rel, full in items:
        ...
        if has_ref:
            icon_type_val = "image"
            has_ref_count += 1
        elif meta and meta.get("icon_type") == "emoji":
            icon_type_val = "emoji"
            no_ref_count += 1
        elif meta:
            icon_type_val = meta.get("icon_type") or ""
            no_ref_count += 1
        else:
            icon_type_val = ""
            no_ref_count += 1
        fmt = os.path.splitext(rel)[1].lstrip(".").upper()  # PNG, SVG
        rows.append((rel, name, et, cat, label, color_badge, icon_type_val, source, fmt))
...
lines.append("> **Tip:** View with GitHub dark mode -- icons are white/colored on transparent.")
lines.append("")
lines.append(f"**Total icons:** {len(all_icons)}")
lines.append(f"- **{has_ref_count}** with `icon_type: image` in stats_definitions.json")
lines.append(f"- **{no_ref_count}** with `icon_type: emoji` or not referenced")
lines.append("")
lines.append("**Source**: `iOS` = from iOS app assets, `doxx-www` = created for web portal, `style` = SVG created in this repo")
lines.append("")
...
for group_name in sorted(group_lines.keys()):
    ...
    lines.append("")
    lines.append("| Icon | Name | Label | Color | Format | icon_type | Source |")
    lines.append("|------|------|-------|-------|--------|-----------|--------|")
    for rel, name, et, cat, label, color_badge, icon_type_val, source, fmt in group_lines[group_name]:
        et_cat = f"{et}.{cat}" if cat else et
        src_short = "style" if source == "style (created)" else source
        lines.append(f"| <img src=\"{rel}\" width=\"24\" height=\"24\"> | `{et_cat or name}` | {label} | {color_badge} | {fmt} | {icon_type_val} | {src_short} |")
    lines.append("")
